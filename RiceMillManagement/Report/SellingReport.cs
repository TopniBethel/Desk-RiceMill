...
namespace RiceMillManagement.Report
{
    public partial class SellingReport : Form
    {
        ...
        public SellingReport(string login)
        {
        ...
        {
            string qry = "select id as Id,sno as SNo,stockfor as StockFor,grade as Grade,name as Name,weight as Total,date as Date,LTRIM(RIGHT(convert(varchar,time,100),7)) as Time from PaddySellingEntry_tbl where date between '" + Convert.ToDateTime(dateTimePicker1.Text).ToString("yyyy-MM-dd") + "' and '" + Convert.ToDateTime(dateTimePicker2.Text).ToString("yyyy-MM-dd") + "' order by id desc";
